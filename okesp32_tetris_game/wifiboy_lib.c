...
DRAM_ATTR static const lcd_init_cmd_t lcd_init_cmds_wbr[]={
    /* Soft Reset */
    {0x1, {0}, 0x80},
    /* Sleep out */
    {0x11, {0}, 0x80},
...
void lcd_init(int btype) 
{
    ...
    switch(_board_type) {
        ...
        case 3: lcd_init_cmds = lcd_init_cmds_wbr; break;
    }
...
void wb_init(int t)
{
    ...
    */
    ...
    //wb_fillScreen(0xf8); //red
	wb_fillScreen(0x00);   //black
...
static UINT infunc(JDEC *decoder, BYTE *buff, UINT nd)
{
    ...
    if (buff != NULL) {
        memcpy(buff, jd->inData + jd->inPos, nd);
    }
    jd->inPos += nd;
    return nd;
    /*
...
esp_err_t decode_image(uint8_t *buf) 
{
    char *work=NULL;
    int r;
    JDEC decoder;
    ...
    jd.inData=buf; // not used for file decoding
    jd.inPos=0;
